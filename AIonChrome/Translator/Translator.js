 ...
 * (X) Translator.create() 메서드를 setTimeout callback 메서드 안에서 실행
 * (O) 특정 버튼을 클릭 시 Trnaslator.create()
 * 메서드를 실행함
 *
...
document.querySelector("button").addEventListener("click", async () => {
  if ("Translator" in self) {
    console.log("현재 브라우저에서 Translator API를 지원합니다!"); // 브라우저가 Tranlator API 제공하는지 체크
  }
  
  const availability = await Translator.availability({
    sourceLanguage: "ko", // 원본 언어 코드
    targetLanguage: "en", // 번역 대상 언어 코드
  });
  ...
  const translator = await Translator.create({
      ...
      monitor(m) {
        m.addEventListener("downloadprogress", (e) => {
          console.log(`다운로드 진행률 ${Math.floor(e.loaded * 100)}%`);
        });
      },
  });

  const inputText = "안녕하세요!"; // 원본 텍스트

  const resultText = await translator.translate(inputText); // 번역
